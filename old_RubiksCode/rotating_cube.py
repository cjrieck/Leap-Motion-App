...
class Simulation:
	def __init__(self, leapController, leapListener, win_width = 640, win_height = 480):
		pygame.init()

		self.screen = pygame.display.set_mode((win_width, win_height))
		pygame.display.set_caption("Leap Rotating Rubik's Cube")
		
		self.clock = pygame.time.Clock()

		self.controller = leapController
	...
	def run(self):
		...
		while True:
			...
			if 'actualX' in listener.storage:
				self.angle = listener.storage['actualX']
			else:
				self.angle += 1
...
if __name__ == "__main__":
	...
	Simulation(controller, listener).run()